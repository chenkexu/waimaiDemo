...
//				longitude = Double.valueOf(sp.getString(KEY.CONFIG.LONGITUDE, "-1"));
				latitude = Double.valueOf((Double) SharedPreferenceUtil.get(context, Constants.Key.LATITUDE,"-1"));
				longitude = Double.valueOf((Double) SharedPreferenceUtil.get(context, Constants.Key.LONGITUDE,"-1"));
				break;
			}
			case baseUserLocation: {
				latitude = Double.valueOf((String) SharedPreferenceUtil.get(context,Constants.Key.MY_LATITUDE,"-1"));
				longitude = Double.valueOf((String) SharedPreferenceUtil.get(context,Constants.Key.MY_LONGITUDE,"-1"));
				break;
			}
			default: {
				latitude = -1;
