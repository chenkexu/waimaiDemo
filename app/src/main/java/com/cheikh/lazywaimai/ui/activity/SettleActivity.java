...
public class SettleActivity extends BaseActivity<OrderController.OrderUiCallbacks>
    ...
    AddressView mAddressInfoView; //地址View

    @Bind(R.id.txt_online_payment)  //在线支付
    TextView mOnlinePaymentTxt;

    @Bind(R.id.txt_offline_payment)
    TextView mOfflinePaymentTxt; //货到付款

    @Bind(R.id.layout_send_time) //送达时间
    View mSendTimeLayout;

    @Bind(R.id.txt_booked_time) //立即送出
    TextView mBookedTimeTxt;

    @Bind(R.id.orderReportView) //订单信息模块
    ...
    View mRemarkLayout; //备注模块

    @Bind(R.id.txt_remark)
    TextView mRemarkTxt;  //备注的信息
    ...
    protected void initializeViews(Bundle savedInstanceState) {
        mSendTimeAdapter = new SendTimeListAdapter(); //选择时间
    ...
    public void onClick(View view) {
        switch (view.getId()) {
            case R.id.view_address_info:
                ...
                break;
            case R.id.txt_online_payment:  //点击在线支付
                showLoading(R.string.label_being_something);
                getCallbacks().togglePayMethod(true);
                break;
            case R.id.txt_offline_payment:  //货到付款
                showLoading(R.string.label_being_something);
                getCallbacks().togglePayMethod(false);
                break;
            case R.id.btn_submit_order: //确认下单（从购物车得到各种信息）
                showLoading(R.string.label_being_something);
